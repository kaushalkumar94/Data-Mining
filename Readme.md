...
│       └── metrics_heatmap.png
├── lab11/
│   ├── Anomaly_Detection_Model_Comparison.ipynb
│   ├── README.md
│   └── outputs/
│       ├── scatter_plot.png
│       ├── f1_score.png
│       └── time_comparison.png
└── lab12/
    ├── Ensemble_Learning_Comparison.ipynb
    ├── README.md
    └── outputs/
        ├── Breast_Cancer_accuracy.png
        ├── Breast_Cancer_f1.png
        ├── Breast_Cancer_time.png
        ├── Iris_accuracy.png
        ├── Iris_f1.png
        ├── Iris_time.png
        └── comparison.png
...
**Files:** `lab11/Anomaly_Detection_Model_Comparison.ipynb`, `lab11/README.md`, `lab11/outputs/`

A comparison of **3 unsupervised anomaly detection algorithms** on a synthetic 2D dataset (300 normal points in 2 clusters + 30 injected outliers). Models evaluated on F1 Score and computation time.
...
Key findings: Isolation Forest and LOF both achieve the best F1 Score (0.76), outperforming One-Class SVM (0.65). However, Isolation Forest is ~40× slower due to building an ensemble of 100 trees. LOF offers the best trade-off — high accuracy and fast computation.

---

### Lab 12 — Ensemble Learning Model Comparison
**Files:** `lab12/Ensemble_Learning_Comparison.ipynb`, `lab12/README.md`, `lab12/outputs/`

A comprehensive comparison of **7 classifiers** (2 baselines + 5 ensemble methods) across two real-world datasets (Breast Cancer and Iris), evaluated on Accuracy, weighted F1 Score, and Training Time.

Algorithms compared:
- Logistic Regression, Decision Tree *(baselines)*
- Bagging, Random Forest, AdaBoost, Gradient Boosting, Voting Classifier *(ensembles)*

Key findings: All models achieve ~95–97% F1 on Breast Cancer; most reach 1.00 on Iris. AdaBoost is the weakest on Iris (~0.93) with default settings. Gradient Boosting and Random Forest are the most consistent performers. Voting Classifier is the slowest due to training multiple sub-models internally.
